...
notE :: [(String, Double)]
notE = [
  ("LAT168-1", 0.30), ("LAT171-1", 0.43), ("LAT174-1", 0.65), ("GRP768-1", 0.70), ("LAT142-1", 0.70),
  ("GRP505-1", 0.74), ("LAT164-1", 0.74), ("GRP506-1", 0.78), ("GRP507-1", 0.78), ("LAT148-1", 0.78),
  ("LAT153-1", 0.78), ("LAT155-1", 0.78), ("GRP508-1", 0.83), ("LAT162-1", 0.83), ("ALG246-1", 0.87),
  ("GRP024-5", 0.87), ("GRP766-1", 0.87), ("LAT146-1", 0.87), ("LAT159-1", 0.87), ("LAT160-1", 0.87),
  ("LAT170-1", 0.87), ("REL022-2", 0.87), ("GRP196-1", 0.91), ("GRP666-3", 0.91), ("GRP666-4", 0.91),
  ("GRP666-5", 0.91), ("LAT157-1", 0.91), ("LAT169-1", 0.91), ("LCL148-10", 0.91), ("REL020-2", 0.91),
  ("REL021-1", 0.91), ("REL021-2", 0.91), ("REL022-1", 0.91), ("REL029-1", 0.91), ("REL033-1", 0.91),
  ("REL033-3", 0.91), ("REL034-1", 0.91), ("REL034-2", 0.91), ("REL035-1", 0.91), ("REL035-2", 0.91),
  ("REL036-1", 0.91), ("GRP164-1", 0.96), ("GRP164-2", 0.96), ("GRP666-2", 0.96), ("GRP678-1", 0.96),
  ("GRP721-1", 0.96), ("GRP725-1", 0.96), ("KLE110-10", 0.96), ("LAT072-1", 0.96), ("LAT076-1", 0.96),
  ("LAT140-1", 0.96), ("LAT141-1", 0.96), ("LAT144-1", 0.96), ("LAT147-1", 0.96), ("LAT151-1", 0.96),
  ("LAT158-1", 0.96), ("LAT163-1", 0.96), ("LAT167-1", 0.96), ("LAT176-1", 0.96), ("LAT183-10", 0.96),
  ("LAT186-10", 0.96), ("LCL927-10", 0.96), ("REL020-1", 0.96), ("REL040-1", 0.96), ("REL040-3", 0.96),
  ("GRP724-1", 1.00), ("LAT074-1", 1.00), ("LAT077-1", 1.00), ("LAT078-1", 1.00), ("LAT079-1", 1.00),
  ("LAT139-1", 1.00), ("REL038-1", 1.00)]
...
readResults = do
  ...
  forM filenames $ \filename -> do
    ...
    let name = takeBaseName directory
    solved <- lines <$> readFile filename
    fast <- filterM (solvedInTime 300 directory) solved
...
main = do
  ...
  forM_ options $ \(option, f) -> do
    forM_ bonuses $ \(bonus, b) -> do
      ...
      forM_ (zip best (inits best)) $ \(name, names) -> do
        putStrLn (name ++ " " ++ show (score results1 (name:names)) ++ ", useful at levels " ++ show (levels results1 name names))

      putStrLn ""

--  putStrLn "\nBest:"
--  forM_ [1..8] $ \i -> do
--    cover <- maxCover i results
--    putStrLn (show i ++ ": " ++ show (score results cover))
--    forM_ cover $ \name -> putStrLn ("  " ++ name)
